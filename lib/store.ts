    ...
    (set, get) => ({
      ...
      getFilteredReminders: () => {
        ...
        return reminders.filter(reminder => {
          const dateMatch = format(reminder.startDateTime, 'yyyy-MM-dd') === 
                           format(selectedDate, 'yyyy-MM-dd');
          const petMatch = !selectedPet || reminder.pet === selectedPet;
          const categoryMatch = !selectedCategory || reminder.category === selectedCategory;
      ...
      getTodayCompletedCount: () => {
        const { reminders } = get();
        return reminders.filter(reminder => 
          isToday(reminder.startDateTime) && reminder.isCompleted
        ).length;
      },

      getTodayTotalCount: () => {
        const { reminders } = get();
        return reminders.filter(reminder => isToday(reminder.startDateTime)).length;
      },
